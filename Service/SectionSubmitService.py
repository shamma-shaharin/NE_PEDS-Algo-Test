...
def Submit_Section_A(driver, row):
    A3_Birthdate_Picker_Element = driver.find_element_by_name("answers[5].value")
    A3_Birthdate_Picker_Element.clear()
    A3_Birthdate = Format_Date(row["A3_Birthdate"])
    A3_Birthdate_Picker_Element.send_keys(A3_Birthdate)
    A3_Birthdate_Picker_Element.click()
...
def Submit_Section_I(driver, row):
    ...
    I1p_Congenital_heart_disorder = driver.find_element_by_xpath(
        "//input[@name='answers[17].refData' and @value='" + AdjustValue(row["I1p_Congenital_heart_disorder"], 0) + "']")

...
from Model.PESDAlgoScore import *
...
def Get_Score_And_Determine_Output(driver, formId, output, row, rowNumber):
    NF_LOC_Child_Value = driver.find_elements_by_xpath(
        "//table[@class='table table-bordered table-striped ']/tbody/tr[1]/td[2]")[0].text
    NF_LOC_Child_Value_Label = driver.find_elements_by_xpath(
        "//table[@class='table table-bordered table-striped ']/tbody/tr[1]/td[3]")[0].text

    LOC_Child_Met_Reason_Value = driver.find_elements_by_xpath(
        "//table[@class='table table-bordered table-striped ']/tbody/tr[3]/td[2]")[0].text
    LOC_Child_Met_Reason_Value_Label = driver.find_elements_by_xpath(
        "//table[@class='table table-bordered table-striped ']/tbody/tr[3]/td[3]")[0].text

    expectedOutput = row["Expected_Output"]
    actualOutput = NF_LOC_Child_Value + "-" + LOC_Child_Met_Reason_Value
    testStatus = expectedOutput == actualOutput
    output.append(PEDSAlgorithmScore(formId, NF_LOC_Child_Value, LOC_Child_Met_Reason_Value, testStatus, expectedOutput,
                                     actualOutput, rowNumber))
